...
class FooEnv(gym.Env):
    ...
    def step(self, action):
        ...
        power_2 = mt.pow(10, ((action[2]-1)*30/10+3)) # power for UT 2
        power_3 = mt.pow(10, ((action[3]-1)*30/10+3)) # power for UT 3
        Theta_R = action[4:] * 2 * np.pi

        step = globe.get_value('step')
        reward, radio_state, received_energy = self.env_state(step, tau, power_1, power_2, power_3, Theta_R)
        terminated = step == t - 1
        truncated = False
    ...
    def reset(self, *, seed=None, options=None):
        ...
        UT_1 = globe.get_value('UT_1')[step]
        UT_2 = globe.get_value('UT_2')[step]
        ...
        distance_RIS_UT_1 = mt.sqrt(mt.pow((L_U[0] - UT_1[0]), 2) + mt.pow((L_U[1] - UT_1[1]), 2) + mt.pow((L_U[2] - UT_1[2]), 2))
    ...
    def EH(self, tau, power_1, power_2, power_3, Theta_R, L_U, L_AP):
        ...
        E_t = tau * eta * received_power
        #print(tau)

        return E_t, received_power * 1
    ...
    def capacity (self, tau, power_1, power_2, power_3, Theta_R, L_U, L_AP, UT_0, UT_1, UT_2):
        ...
        # received signal for UT 2
        h_ru = self.Channel_RU(L_U, UT_1, BS_Z, RIS_L)    
        UT_link_2 = 1*np.linalg.multi_dot([G, coefficients, h_ru])
        signal_UT_2 = np.sum(np.abs(UT_link_1 * np.conjugate(UT_link_2))) * power_2

        # received signal for UT 3
        h_ru = self.Channel_RU(L_U, UT_2, BS_Z, RIS_L)    
        UT_link_3 = 1*np.linalg.multi_dot([G, coefficients, h_ru])
        signal_UT_3 = np.sum(np.abs(UT_link_3 * np.conjugate(UT_link_3))) * power_3

        SINR_1 = 10 * mt.log(max(float(signal_UT_1) / max(float(AWGN), EPS), EPS), 10)
        SINR_2 = 10 * mt.log(max(float(signal_UT_2) / max(float(AWGN), EPS), EPS), 10)
        SINR_3 = 10 * mt.log(max(float(signal_UT_3) / max(float(AWGN), EPS), EPS), 10)
        ...
        if SINR_2 > 0:
            Aver_Throughput_2 = BW * mt.log((1 + SINR_2), 2) * (1 - tau)
        else:
            Aver_Throughput_2 = 0

        if SINR_3 > 0:
            Aver_Throughput_3 = BW * mt.log((1 + SINR_3), 2) * (1 - tau)
        else:
            Aver_Throughput_3 = 0

        return [Aver_Throughput_1, Aver_Throughput_2, Aver_Throughput_3]

    def env_state(self, step, tau, power_1, power_2, power_3, Theta_R):
        if step < globe.get_value('t')-1:
            L_U = globe.get_value('UAV_Trajectory')[step+1]
            L_AP = globe.get_value('L_AP')

            UT_0 = globe.get_value('UT_0')[step+1]
            UT_1 = globe.get_value('UT_1')[step+1]
            UT_2 = globe.get_value('UT_2')[step+1]
        else:
            L_U = globe.get_value('UAV_Trajectory')[step]
            L_AP = globe.get_value('L_AP')
        ...
        reward, received_energy = self.EH(tau, power_1, power_2, power_3, Theta_R, L_U, L_AP)
        HarvestEnergy = reward
        Aver_Throughput = self.capacity (tau, power_1, power_2, power_3, Theta_R, L_U, L_AP, UT_0, UT_1, UT_2)
        decline = reward * 1/3

        for i in range(len(Aver_Throughput)):
            if Aver_Throughput[i] < 7 * mt.pow(10, 7):
                reward = 0#reward - decline
                HarvestEnergy = 0

        #reward = reward * pow(2, l-1)
